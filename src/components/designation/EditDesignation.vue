...
// fetch single designation using id
getRecordApi(`/designation/${props.id}`).then((res: any) => {
...
const handleValidateClick = (e: MouseEvent) => {
  ...
  formRef.value?.validate((errors) => {
    if (!errors) {
      ...
      updateRecordApi(`/designation/${formValue.value.id}`, formValue.value).then((res: any) => {

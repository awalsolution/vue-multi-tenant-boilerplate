<template>
  <n-form ref="formRef" :label-width="80" :model="formValue" :rules="rules" size="small">
    <n-form-item style="padding-top: 24px" label="Permission Name" path="name">
      <n-input v-model:value="formValue.name" placeholder="Enter Permission Name" />
    </n-form-item>
    <n-form-item style="padding-top: 4px" label="Permission Description" path="description">
      <n-input v-model:value="formValue.description" placeholder="Enter Permission Description" />
    </n-form-item>
  ...
  const handleValidateClick = (e: MouseEvent) => {
    ...
    formRef.value?.validate((errors) => {
      if (!errors) {
        ...
        createPermissionApi({ name, description }).then((result) => {

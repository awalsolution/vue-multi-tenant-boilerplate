<template>
  <div>
    <DataTable class="" :value="list" stripedRows dataKey="id" scrollable paginator :rows="20">
      ...
      <Column field="profile_picture" header="Image" class="">
        <template #body="{ data }">
      ...
      <Column field="email" header="Email" class="whitespace-nowrap">
        <template #body="{ data }">
          {{ data?.email }}
        </template>
      </Column>
      ...
      >
        <template #body="{ data }">
          <Button
            ...
            class="mr-2"
            @click="
              router.push({ name: 'organization_details', params: { db_name: data?.db_name } })
            "

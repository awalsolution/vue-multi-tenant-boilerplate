<template>
  <ContentLayout>
    <template #contentHeader>
      <h1 class="text-3xl font-bold bg-slate-200 py-2 text-center text-blue-400 mb-3">
        Tenant Information
      </h1>
    </template>
    <n-tabs
    ...
    >
        ...
        <DataTableLayout :loading="loading">
          ...
          <template #content>
            <div class="px-10 pt-5 w-full">
              <div
                class="bg-white rounded-lg shadow-lg w-full overflow-x-scroll border border-gray-200"
              >
                <div
                  class="flex gap-3 flex-col sm:flex-row flex-wrap w-full items-center sm:w-fit p-3"
                >
                  <n-input
                    class="sm:!w-[230px]"
      ...
      <n-tab-pane name="user" tab="User"> Total Users: {{ users.length }} </n-tab-pane>
    </n-tabs>
  </ContentLayout>
</template>
...
const { getList, list, page, pageSizes, itemCount, perPage, searchParams }: any = usePagination(
  `/role/tenant/${route.query.tenant_id}`
);
...
console.log('query==>', route.query.tenant_id);

// onMounted(async () => {
//   if (route.query && route.query.tenant_id) {
//     getRecordsApi(`/user/tenant/${route.query.tenant_id}`, { perPage: 14 }).then((res: any) => {
//       users.value = res?.data?.data;
//     });
//     getRecordsApi(`/role/tenant/${route.query.tenant_id}`, { perPage: 14 }).then((res: any) => {
